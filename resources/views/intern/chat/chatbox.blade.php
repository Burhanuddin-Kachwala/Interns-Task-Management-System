<x-layout>
    <div class="max-w-4xl mx-auto mt-10 bg-white shadow rounded p-4">
        ...
        <div class="h-96 overflow-y-scroll border p-4 mb-4 space-y-2 bg-gray-50 rounded">
            @foreach ($messages as $msg)
                <div class="flex {{ $msg->sender_type === 'intern' ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-xs px-4 py-2 rounded {{ $msg->sender_type === 'intern' ? 'bg-green-500 text-white' : 'bg-gray-200' }}">
                        <p>{{ $msg->message }}</p>
        ...
        <form method="POST" action="{{ route('intern.chat.send', $admin->id) }}" class="flex space-x-2">
            @csrf
            <input type="text" name="message" class="w-full border rounded px-3 py-2" placeholder="Type your message...">
            <button type="submit" class="bg-green-600 text-white px-4 rounded hover:bg-green-700">Send</button>
        </form>
    </div>
</x-layout>
