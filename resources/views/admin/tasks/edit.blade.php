<x-admin.layout>
    ...
    <form id="admin_edit_task" action="{{ route('admin.tasks.update', $task->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" class="w-full px-4 py-2 border rounded summernote">{!! $task->description !!}</textarea>
        </div>

        <div>
            <label class="block font-medium">Deadline</label>
            <input type="date" name="deadline" value="{{ $task->deadline }}" class="w-full px-4 py-2 border rounded" required>
        </div>
        ...
        <div>
            ...
            <select name="interns[]" multiple class="w-full border rounded px-2 py-1">
                @foreach ($interns as $intern)
                    <option value="{{ $intern->id }}" @if($task->interns->contains($intern->id)) selected @endif>
                        {{ $intern->name }}
                    </option>
                @endforeach
            </select>
        ...
        <button type="submit" class="bg-indigo-500 text-white px-4 py-2 rounded hover:bg-indigo-600">Update Task</button>
    </form>
</x-admin.layout>
